...
<template>
  <div class="w-full h-full">
    <header ref="referenceRef" class="relative z-50 ">
        ...
        <ul class="hidden md:flex px-6 py-2 bg-primary-800 border-b border-b-neutral-200 border-b-solid" @blur="(event:any) => {
          ...
          <li v-for="(menuNode, index) in content.children" :key="menuNode.key">
            ...
            <div v-if="isOpen && activeNode.length === 1 && activeNode[0] === menuNode.key" :key="activeMenu.key"
              ref="megaMenuRef" :style="style"
              class="hidden md:grid gap-x-6 grid-cols-4 bg-white shadow-lg p-6 left-0 right-0 outline-none" tabindex="0"
              @mouseleave="close()" @keydown.esc="focusTrigger(index)">
      ...
      <SfDrawer ref="drawerRef" v-model="isOpen" placement="left"
        ...
        <nav>
          ...
          <ul class="mt-2 mb-6">
            <li v-if="activeMenu.key !== 'root'">
              <SfListItem size="lg" tag="button" type="button" class="border-b border-b-neutral-200 border-b-solid"
                @click="goBack()">
                <div class="flex items-center">
          ...
          <div v-if="bannerNode.value.banner"
            class="flex items-center overflow-hidden bg-neutral-100 border-neutral-300 grow">
            <img :src="bannerNode.value.banner" :alt="bannerNode.value.bannerTitle"
